...
const qaq = artifacts.require('Gartic2');

contract('Gartic2', function (accounts) {
	...
	context('fonctions', function () {
		...
		it('... tester la function StoreWord', async () => {
			...
			await expectRevert(Gartic.StoreWord(newWord, { from: j2 }), "Can't write same word !");
			await expectRevert(Gartic.StoreWord("N'impote", { from: j1 }), 'tas deja joue!');
	...
	context('tous les joueurs ont joue', function () {
		...
		it('... tester la function getFirstLast', async () => {
			await expectRevert(Gartic.getFirstLast({ from: owner }), 'jeu pas encore fini');
			await Gartic.StoreWord(newWord + 5, { from: j5 });
		...
		it('... tester la function guessIt', async () => {
			await expectRevert(Gartic.guessIt('UnMot', { from: j2 }), 'Pas à la bonne étape du jeu');
			await Gartic.StoreWord(newWord + 5, { from: j5 });

			const adrressPerdantZero = await Gartic.guessIt.call('MauvaiseMot', { from: j2 });
			expect(adrressPerdantZero).to.be.bignumber.equal(constants.ZERO_ADDRESS);

			const adrressGagnant = await Gartic.guessIt.call(newWord + 1, { from: j2 });
			expect(adrressGagnant).to./* be.bignumber. */ equal(j2);
		});

		it('... tester la function getResults', async () => {
			await expectRevert(Gartic.getResults(), 'jeu pas encore fini');
			await Gartic.StoreWord(newWord + 5, { from: j5 });
		...
		it('... tester la function resetGame', async () => {
			...
			// assert.equal(words.length, 0, "doit etre 0");
		});
	});
});
